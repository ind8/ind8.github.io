...
if [ $name != "metasploit.sh" ]; then
	echo "[-] jangan gunain third-party stolen scripts"
	exit 1
fi
...
if [ -d "$msfpath/metasploit-framework" ]; then
	echo "metasploit sudah diinstall broh"
	exit 1
fi
apt update
...
echo "Gemsnya di Install dlu ya!"
...
echo "gunain msfvenom atau msfconsole dari pada ./msfvenom atau ./msfconsole sudah di symlinked di $PREFIX/bin"
